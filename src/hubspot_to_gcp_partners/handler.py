...
class HubSpotToGcpPartnersHandler(BaseLambdaHandler):
    """Handler for HubSpot webhook events → GCP Partners API sync."""

    def _execute(self, event: dict, context: dict) -> dict:
        """Process HubSpot webhook events and sync to GCP Partners API."""
        try:
        ...
        for webhook_event in webhook_events:
            ...
            try:
                if "deal.creation" in event_type:
                    result = self._handle_deal_creation(
                        object_id, gcp_client, partner_id
                    )
                elif "deal.propertyChange" in event_type:
                    result = self._handle_deal_update(
                        object_id, webhook_event, gcp_client, partner_id
                    )
                else:
    ...
    def _handle_deal_creation(
        self, deal_id: str, gcp_client, partner_id: str
    ) -> dict | None:
        """
        ...
        self.logger.info("Created GCP lead %s for deal %s", lead_name, deal_id)
        ...
        opportunity_id = (
            opportunity_name.split("/")[-1]
            if "/" in opportunity_name
            else opportunity_name
        )
        self.logger.info(
            "Created GCP opportunity %s for deal %s", opportunity_id, deal_id
        )
    ...
    def _handle_deal_update(
        ...
        if not opportunity_id or not opportunity_name:
            self.logger.debug(
                "Deal %s has no gcp_opportunity_id — skipping update", deal_id
            )
            return None
        ...
        current_gcp_opp = self._get_gcp_opportunity(gcp_client, opportunity_name)
        ...
        self.logger.info(
            "Updated GCP opportunity %s from deal %s", opportunity_id, deal_id
        )

        self.hubspot_client.update_deal(deal_id, {"gcp_sync_status": "synced"})
    ...
    def _get_gcp_opportunity(self, gcp_client, opportunity_name: str) -> dict | None:
        """Fetch a GCP opportunity; return None on failure."""
        try:
            return (
                gcp_client.partners()
                .opportunities()
                .get(name=opportunity_name)
                .execute()
            )
        except Exception as exc:
            self.logger.error(
                "Failed to fetch GCP opportunity %s: %s", opportunity_name, exc
            )
            return None
...
_handler = HubSpotToGcpPartnersHandler()
