...
def lambda_handler(event: dict, context: dict) -> dict:
    ...
    """
    try:
        logger.info("Starting engagement lifecycle sync")
        ...
        deals = hubspot_client.get_all_deals_with_property("aws_opportunity_id")
        logger.info(f"Found {len(deals)} deals with AWS opportunities")
        ...
        for deal in deals:
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to list engagements for {opportunity_id}: {e}")
                ...
                if not engagements:
                    logger.debug(f"No engagements found for opportunity {opportunity_id}")
                    skipped_count += 1
                    continue
                ...
                try:
                    engagement = pc_client.get_engagement(
                        Catalog="AWS",
                ...
                team_members = []
                try:
                    members_response = pc_client.list_engagement_members(
                        Catalog="AWS",
                        Identifier=engagement_id
                    )
                    team_members = members_response.get("EngagementMemberList", [])
                except Exception as e:
                    logger.warning(f"Failed to get engagement members: {e}")
                ...
                team_emails = [
                    m.get("Email") for m in team_members
                    if m.get("Email")
                ]
                ...
                update_properties = {
                    ...
                    "aws_last_engagement_sync": hubspot_client.now_timestamp_ms()
                }
                
                if created_date:
                    update_properties["aws_engagement_kickoff_date"] = _iso_to_hubspot_timestamp(created_date)
                
                # Remove None values
                update_properties = {k: v for k, v in update_properties.items() if v is not None}
                
                hubspot_client.update_deal(deal_id, update_properties)
                
                # Create timeline event if status changed
                if status_changed:
                    _create_status_change_timeline_event(
                        hubspot_client,
                        deal_id,
                        current_status,
                        engagement_status
                    ...
                    # Also create a note
                    note_text = _create_engagement_status_note(
                        engagement_status,
                        team_emails
                    )
                ...
                synced_count += 1
                logger.info(f"Synced engagement {engagement_id} to deal {deal_id}")
                
            except Exception as e:
                logger.error(f"Error syncing deal {deal.get('id')}: {e}", exc_info=True)
        ...
        logger.info(f"Engagement lifecycle sync complete: {json.dumps(result)}")
        ...
        logger.error(f"Fatal error in engagement lifecycle sync: {e}", exc_info=True)
        return {
            "statusCode": 500,
...
def _iso_to_hubspot_timestamp(iso_string: str) -> int:
...
def _create_status_change_timeline_event(
    ...
    if old_status:
        description = f"Engagement status changed from {old_status} to {new_status}"
    else:
        description = f"Engagement status: {new_status}"
    
    # Note: HubSpot timeline events require custom event definition
    # For now, we'll just create a note (done in main handler)
    logger.info(f"Would create timeline event: {title}")
