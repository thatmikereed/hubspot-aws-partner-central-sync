...
def hubspot_deal_to_gcp_lead(
    ...
    if associated_contacts:
        ...
        primary_contact = {
            ...
            "phone": _sanitize_phone(contact_props.get("phone") or contact_props.get("mobilephone")),
        }
        # Remove empty fields
...
def hubspot_deal_to_gcp_opportunity(
    ...
    notes = (props.get("description") or props.get("hs_deal_description") or "").strip()
    next_steps = (props.get("hs_next_step") or props.get("notes_next_activity_description") or "").strip()
...
def gcp_opportunity_to_hubspot_deal(gcp_opportunity: dict, gcp_lead: Optional[dict] = None) -> dict:
    """
    ...
    base_name = gcp_lead.get("companyName", "GCP Partner Opportunity") if gcp_lead else "GCP Partner Opportunity"
...
def hubspot_deal_to_gcp_opportunity_update(
    ...
    if not changed_properties or "amount" in changed_properties:
        ...
        try:
            deal_size = float(amount_str)
            if deal_size > 0:
                update_payload["dealSize"] = deal_size
        except (ValueError, TypeError):
            pass
