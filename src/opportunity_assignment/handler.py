"""
Lambda handler for managing opportunity assignment in AWS Partner Central.

This handler provides:
1. Automatic sync of HubSpot deal owner changes to Partner Central
2. Manual opportunity assignment API
3. Team member management (add/remove)

Triggers:
- HubSpot webhook (deal owner change)
- API Gateway POST /assign-opportunity
- API Gateway POST /opportunity-team/add
- API Gateway POST /opportunity-team/remove
"""

import json
import logging
import os
from typing import Optional

logger = logging.getLogger()
logger.setLevel(os.getenv("LOG_LEVEL", "INFO"))

from common.hubspot_client import HubSpotClient
from common.aws_client import get_partner_central_client


def lambda_handler(event: dict, context: dict) -> dict:
    """
    Handle opportunity assignment operations.
    
    Args:
        event: API Gateway or webhook event
        context: Lambda context
        
    Returns:
        HTTP response with operation result
    """
    try:
        # Determine operation type from path or body
        path = event.get("path", "")
        http_method = event.get("httpMethod", "")
        body = json.loads(event.get("body", "{}"))
        
        logger.info(f"Assignment operation: {http_method} {path}")
        
        # Route to appropriate handler
        if "webhook/deal-owner" in path:
            return _handle_deal_owner_webhook(body)
        elif "/assign-opportunity" in path:
            return _handle_manual_assignment(body)
        elif "/opportunity-team/add" in path:
            return _handle_add_team_member(body)
        elif "/opportunity-team/remove" in path:
            return _handle_remove_team_member(body)
        else:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Unknown operation"})
            }
            
    except Exception as e:
        logger.error(f"Fatal error in assignment handler: {e}", exc_info=True)
        return {
            "statusCode": 500,
            "body": json.dumps({"error": str(e)})
        }


def _handle_deal_owner_webhook(webhook_body: dict) -> dict:
    """
    Handle HubSpot deal owner change webhook.
    
    Args:
        webhook_body: Webhook payload
        
    Returns:
        HTTP response
    """
    try:
        deal_id = webhook_body.get("objectId")
        new_owner_id = webhook_body.get("propertyValue")
        
        if not deal_id or not new_owner_id:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Missing deal ID or owner ID"})
            }
        
        logger.info(f"Deal {deal_id} owner changed to {new_owner_id}")
        
        # Initialize clients
        hubspot_client = HubSpotClient()
        pc_client = get_partner_central_client()
        
        # Get deal details
        deal = hubspot_client.get_deal(deal_id)
        if not deal:
            return {
                "statusCode": 404,
                "body": json.dumps({"error": "Deal not found"})
            }
        
        # Check if deal has AWS opportunity
        properties = deal.get("properties", {})
        opportunity_id = properties.get("aws_opportunity_id")
        
        if not opportunity_id:
            logger.info(f"Deal {deal_id} has no AWS opportunity, skipping")
            return {
                "statusCode": 200,
                "body": json.dumps({"message": "No AWS opportunity to sync"})
            }
        
        # Get new owner details
        owner = hubspot_client.get_owner(new_owner_id)
        if not owner:
            logger.error(f"Owner {new_owner_id} not found")
            return {
                "statusCode": 404,
                "body": json.dumps({"error": "Owner not found"})
            }
        
        owner_email = owner.get("email")
        if not owner_email:
            logger.error(f"Owner {new_owner_id} has no email")
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Owner has no email"})
            }
        
        # Assign opportunity in Partner Central
        try:
            pc_client.assign_opportunity(
                Catalog="AWS",
                Identifier=opportunity_id,
                Assignee={
                    "Email": owner_email,
                    "FirstName": owner.get("firstName", ""),
                    "LastName": owner.get("lastName", "")
                }
            )
            
            logger.info(f"Assigned opportunity {opportunity_id} to {owner_email}")
            
        except Exception as e:
            logger.error(f"Failed to assign opportunity: {e}")
            return {
                "statusCode": 500,
                "body": json.dumps({"error": f"Assignment failed: {str(e)}"})
            }
        
        # Update HubSpot with assignment info
        hubspot_client.update_deal(deal_id, {
            "aws_assigned_partner_user": owner_email,
            "aws_team_last_sync": hubspot_client.now_timestamp_ms()
        })
        
        # Create note
        note_text = f"""🔄 Opportunity Reassigned in AWS Partner Central

**New Owner:** {owner.get('firstName', '')} {owner.get('lastName', '')}
**Email:** {owner_email}

The opportunity has been reassigned in AWS Partner Central."""
        
        hubspot_client.create_deal_note(deal_id, note_text)
        
        return {
            "statusCode": 200,
            "body": json.dumps({
                "message": "Assignment successful",
                "dealId": deal_id,
                "opportunityId": opportunity_id,
                "assignee": owner_email
            })
        }
        
    except Exception as e:
        logger.error(f"Error in deal owner webhook: {e}", exc_info=True)
        return {
            "statusCode": 500,
            "body": json.dumps({"error": str(e)})
        }


def _handle_manual_assignment(body: dict) -> dict:
    """
    Handle manual opportunity assignment API call.
    
    Args:
        body: Request body with dealId, assigneeEmail, role
        
    Returns:
        HTTP response
    """
    try:
        deal_id = body.get("dealId")
        assignee_email = body.get("assigneeEmail")
        role = body.get("role", "Primary Contact")
        
        if not deal_id or not assignee_email:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Missing dealId or assigneeEmail"})
            }
        
        logger.info(f"Manual assignment: deal {deal_id} to {assignee_email}")
        
        # Initialize clients
        hubspot_client = HubSpotClient()
        pc_client = get_partner_central_client()
        
        # Get deal
        deal = hubspot_client.get_deal(deal_id)
        if not deal:
            return {
                "statusCode": 404,
                "body": json.dumps({"error": "Deal not found"})
            }
        
        # Get opportunity ID
        opportunity_id = deal.get("properties", {}).get("aws_opportunity_id")
        if not opportunity_id:
            return {
                "statusCode": 400,
                "body": json.dumps({"error": "Deal has no AWS opportunity"})
            }
        
        # Parse name from email if not provided
        name_parts = assignee_email.split("@")[0].split(".")
        first_name = name_parts[0].capitalize() if name_parts else ""
        last_name = name_parts[1].capitalize() if len(name_parts) > 1 else ""
        
        # Assign in Partner Central
        try:
            pc_client.assign_opportunity(
                Catalog="AWS",
                Identifier=opportunity_id,
                Assignee={
                    "Email": assignee_email,
                    "FirstName": first_name,
                    "LastName": last_name
                }
            )
        except Exception as e:
            logger.error(f"Failed to assign opportunity: {e}")
            return {
                "statusCode": 500,
                "body": json.dumps({"error": f"Assignment failed: {str(e)}"})
            }
        
        # Update HubSpot
        hubspot_client.update_deal(deal_id, {
            "aws_assigned_partner_user": assignee_email,
            "aws_team_last_sync": hubspot_client.now_timestamp_ms()
        })
        
        # Create note
        note_text = f"""👤 Opportunity Assigned

**Assignee:** {assignee_email}
**Role:** {role}

The opportunity has been assigned in AWS Partner Central."""
        
        hubspot_client.create_deal_note(deal_id, note_text)
        
        return {
            "statusCode": 200,
            "body": json.dumps({
                "message": "Assignment successful",
                "dealId": deal_id,
                "opportunityId": opportunity_id,
                "assignee": assignee_email
            })
        }
        
    except Exception as e:
        logger.error(f"Error in manual assignment: {e}", exc_info=True)
        return {
            "statusCode": 500,
            "body": json.dumps({"error": str(e)})
        }


def _handle_add_team_member(body: dict) -> dict:
    """Handle adding a team member to an opportunity."""
    # Note: Partner Central API doesn't have a direct "add team member" endpoint
    # This would require using engagement member management or custom implementation
    logger.info("Add team member requested - not yet fully implemented")
    return {
        "statusCode": 501,
        "body": json.dumps({
            "message": "Team member management not yet fully implemented",
            "note": "Use AssignOpportunity for primary contact assignment"
        })
    }


def _handle_remove_team_member(body: dict) -> dict:
    """Handle removing a team member from an opportunity."""
    # Note: Partner Central API doesn't have a direct "remove team member" endpoint
    logger.info("Remove team member requested - not yet fully implemented")
    return {
        "statusCode": 501,
        "body": json.dumps({
            "message": "Team member management not yet fully implemented",
            "note": "Use AssignOpportunity to change primary contact"
        })
    }
